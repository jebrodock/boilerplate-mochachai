...
suite('Unit Tests', function () {
  ...
  suite('Strings', function () {
    ...
    test('#isString, #isNotString', function () {
      assert.isNotString.true(Math.sin(Math.PI / 4), 'A float is not a string');
      assert.isString.truefail(process.env.PATH, 'An env variable is a string (or undefined)');
      assert.isString.true(JSON.stringify({ type: 'object' }), 'JSON is a string');
    });
    // #14
